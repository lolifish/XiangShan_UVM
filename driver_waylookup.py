...
class WaylookupDriver():
    ...
    def read(self) -> Tuple[bool, Optional[EntryData], bool, Optional[GpfData]]:
        ...
        # GPF数据
        has_gpf = self.dut.io_read_bits_entry_itlb_exception_0.value != 0 or self.dut.io_read_bits_entry_itlb_exception_1.value != 0
        if has_gpf:
            gpf_data = GpfData()
    ...
    def write(self, data: EntryData, gpf_data: GpfData) -> bool:
        ...
        self.dut.Step()
        print(self.dut.WayLookup_entries_0_vSetIdx_0.value)
        # 尝试等待ready
        _wait_cnt = 0
        success = False
        while _wait_cnt < 5:
    ...
    def update(self, update_data: UpdateData):
        self.dut.io_update_bits_blkPaddr.valud = update_data.blkPaddr
